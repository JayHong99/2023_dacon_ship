...
class NTXent(nn.Module):
    ...
    def forward(self, z_i, z_j):
        ...
        similarity = F.cosine_similarity(z.unsqueeze(1), z.unsqueeze(0), dim=2).to(device)

        sim_ij = torch.diag(similarity, batch_size).to(device)
        sim_ji = torch.diag(similarity, -batch_size).to(device)
